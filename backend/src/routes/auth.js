...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT id, email, display_name, avatar_data, cover_data, subscription_plan, subscription_expires_at FROM users WHERE id = $1',
      [payload.userId]
    );
...
router.put('/cover', async (req, res) => {
  ...
  if (!cover_data) {
    return res.status(422).json({ error: 'cover_data 為必填' });
  }
  try {
    const payload = jwt.verify(token, process.env.JWT_SECRET);
    await pool.query(
      'UPDATE users SET cover_data = $1 WHERE id = $2',
      [cover_data, payload.userId]
    );
    res.json({ coverData: cover_data });
  } catch {
    res.status(401).json({ error: 'Invalid or expired token' });
  }
});
