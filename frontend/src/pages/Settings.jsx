...
export default function Settings() {
  ...
  const { handleLogout, isLoggedIn, subscriptionPlan, lang, setLang, theme, setTheme, isDark } = useApp();
  const t = getT(lang);
  ...
  const c = isDark ? {
    ...
    submitBg: '#7fb5a0',
    logoutBg: 'rgba(239,68,68,0.12)', logoutBorder: 'rgba(239,68,68,0.3)', logoutText: '#f87171',
    langBtn: 'rgba(255,255,255,0.06)', langBorder: 'rgba(255,255,255,0.1)', langText: '#a8a29e',
  } : {
    ...
    submitBg: '#7fb5a0',
    logoutBg: '#fef2f2', logoutBorder: '#fecaca', logoutText: '#dc2626',
    langBtn: '#f5f3f0', langBorder: '#e8e0d0', langText: '#6b7280',
  };

  return (
    <div style={s.page}>
      <button style={{ ...s.back, color: '#7fb5a0' }} onClick={() => navigate('/account')}>← 返回我的</button>
        ...
        return (
          <div style={{ ...s.section, background: c.card, border: isPro ? '1.5px solid rgba(127,181,160,0.5)' : c.cardBorder }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 99, background: isPro ? 'rgba(127,181,160,0.15)' : (isDark ? 'rgba(255,255,255,0.07)' : '#f3f4f6'), color: isPro ? '#7fb5a0' : c.textMid, border: isPro ? '1px solid rgba(127,181,160,0.3)' : `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#e5e7eb'}` }}>
            ...
            {!isPro && (
              <div style={{ display: 'flex', background: isDark ? 'rgba(255,255,255,0.05)' : '#f3f4f6', borderRadius: 10, padding: 3, marginBottom: 16 }}>
                {['monthly', 'yearly'].map((b) => (
                  <button key={b} onClick={() => setBilling(b)} style={{ flex: 1, border: 'none', borderRadius: 8, padding: '7px 0', fontSize: 13, fontWeight: billing === b ? 600 : 400, cursor: 'pointer', fontFamily: 'inherit', background: billing === b ? (isDark ? 'rgba(127,181,160,0.2)' : '#fff') : 'transparent', color: billing === b ? '#7fb5a0' : c.textMid, boxShadow: billing === b ? '0 1px 4px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s' }}>
            ...
            {!isPro && (
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <span style={{ fontSize: 28, fontWeight: 700, color: '#7fb5a0' }}>{billing === 'monthly' ? 'NT$99' : 'NT$660'}</span>
                <span style={{ fontSize: 13, color: c.textMid, marginLeft: 4 }}>{billing === 'monthly' ? '/ 月' : '/ 年'}</span>
              </div>
            )}
            <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ flex: 1, borderRadius: 12, padding: '12px 14px', background: isPro ? 'rgba(127,181,160,0.08)' : (isDark ? 'rgba(127,181,160,0.07)' : '#f0f9f6'), border: isPro ? '1px solid rgba(127,181,160,0.25)' : '1px solid rgba(127,181,160,0.15)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              {isPro ? (
                <button style={{ ...s.submitBtn, background: 'transparent', color: '#7fb5a0', border: '1.5px solid rgba(127,181,160,0.4)', padding: '12px 32px', width: 'auto' }} onClick={() => window.open(stripeMonthly || '#', '_blank')}>{t.manageBtn}</button>
              ) : (
                <button style={{ ...s.submitBtn, background: upgradeLink ? '#7fb5a0' : '#a8c8be', cursor: upgradeLink ? 'pointer' : 'default', padding: '12px 36px', width: 'auto' }} onClick={() => upgradeLink && window.open(upgradeLink, '_blank')} title={!upgradeLink ? '請設定 VITE_STRIPE_MONTHLY_LINK / VITE_STRIPE_YEARLY_LINK' : undefined}>
      ...
      {isLoggedIn && (
        <div style={{ ...s.section, background: c.card, border: c.cardBorder }}>
          ...
          <button onClick={handleExportCSV} disabled={exporting} style={{ ...s.submitBtn, background: exporting ? '#5a9a87' : c.submitBg, opacity: exporting ? 0.7 : 1 }}>
...
const s = {
  ...
  section: { borderRadius: 14, padding: '20px', marginBottom: 16 },
  sectionLabel: { fontSize: 12, fontWeight: 600, letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 14 },
