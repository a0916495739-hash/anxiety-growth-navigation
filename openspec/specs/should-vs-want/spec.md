...
應該 vs 想要衝突記錄器，幫助使用者記錄內心衝突時刻、標記情緒感受，並以圖表呈現期望來源分佈。
...
系統 SHALL 以時間倒序列表呈現所有衝突記錄，並可刪除個別記錄。
...
- **THEN** 系統呼叫 `DELETE /api/conflicts/:id`，成功後從列表移除，不重新載入整頁

#### Scenario: 無衝突記錄
- **WHEN** 使用者尚未記錄任何衝突
- **THEN** 系統顯示空狀態插圖與提示，引導使用者記錄第一個衝突
